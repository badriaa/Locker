...
public class ViewLevelReportCordActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_view_level_report_cord);

        new GlobalSharedPrefs(this);
        String userid = GlobalSharedPrefs.lockerPref.getString("user_id", "");
        String level = getIntent().getStringExtra("level");

        listView = (ListView) findViewById(R.id.listViewLevelReport);
        new GetLevelLockersTask().execute(userid, level);
    }

    private class GetLevelLockersTask extends AsyncTask<String, Void, String> {
        private SweetAlertDialog progressSignin;

        @Override
        protected void onPreExecute() {
            super.onPreExecute();
            progressSignin = new SweetAlertDialog(ViewLevelReportCordActivity.this, SweetAlertDialog.PROGRESS_TYPE);
        ...
        protected String doInBackground(final String... params) {
            String url = Constants.GET_REPORT_LEVEL_URL + "user_id=" + params[0] + "&level=" + params[1];
            RestClientManager.getInstance().makeJsonRequest(Request.Method.GET,
                    ...
                    new RequestHandler<>(new RequestCallbacks<ReportSuccessResponseModel, ErrorResponseModel>() {
                        ...
                        public void onRequestSuccess(ReportSuccessResponseModel response) {
                            try {
                                ...
                                if (response.status.equalsIgnoreCase("failure"))
                                    PrettyToast.showError(ViewLevelReportCordActivity.this, response.message);

                            } catch (Exception e) {
                                PrettyToast.showError(ViewLevelReportCordActivity.this, "Something went wrong, Please try again");
                                e.printStackTrace();
                            }
                        }
                        ...
                        public void onRequestError(ErrorResponseModel error) {
                            progressSignin.dismiss();
                            PrettyToast.showError(ViewLevelReportCordActivity.this, "There was a problem connecting to server, Please try again");
                        }
